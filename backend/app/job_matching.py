...
SKILLS["Kubernetes"].append("K8s")
SKILLS["PostgreSQL"].append("Postgres")
SKILLS["JavaScript"].append("JS")
SKILLS["TypeScript"].append("TS")
SKILLS["RAG"].append("retrieval-augmented generation")
SKILLS["NLP"].append("natural language processing")
RELATED = {
...
def extract_job(text, extra_skills=()):
    lexicon = {
        **SKILLS,
        **{name: [name] for name in extra_skills if name not in SKILLS},
    }
    required = set()
    preferred = set()
    mode = "required"
    for line in text.splitlines():
        if re.search(r"preferred|nice.to.have|bonus|desirable", line, re.I):
            ...
            r"required|must.have|essential|responsibilities|experience:|education:",
            line,
            re.I,
        ):
            mode = "required"
        found = {
            name
            for name, aliases in lexicon.items()
            if any(has_term(line, alias) for alias in aliases)
        }
    ...
    return {
        ...
        "responsibilities": [
            line.strip()
            for line in text.splitlines()
            if re.search(r"build|design|develop|lead|deliver|responsib", line, re.I)
        ],
        "experience_years": min(years) if years else None,
        "education": [
            line.strip()
            for line in text.splitlines()
            if re.search(r"degree|bachelor|master|ph\.?d|education", line, re.I)
        ],
        "keywords": sorted(
            {
                word.lower()
...
def experience_years(doc):
    ...
    for sec in doc["sections"]:
        if sec["kind"] != "experience" or not sec["visible"]:
            continue
        for item in sec["items"]:
            d = item["data"]
            start = d.get("start_date", "")
            end = today.isoformat() if d.get("current") else d.get("end_date", "")
            try:
                sy = int(start[:4])
                sm = int(start[5:7] or "1")
                ey = int(end[:4])
                em = int(end[5:7] or "12")
                if not (
                    1900 <= sy <= ey <= today.year + 1
                    and 1 <= sm <= 12
                    and 1 <= em <= 12
                ):
                    continue
                months.update(
                    range(
                        sy * 12 + sm,
                        min(ey * 12 + em, today.year * 12 + today.month) + 1,
                    )
                )
...
def match_job(doc, text, items):
    job = extract_job(
        text, [i.data.get("name", "") for i in items if i.kind == "skills"]
    )
    ...
    matched = [
        name
        for name in sought
        if any(has_term(resume_text, alias) for alias in SKILLS.get(name, [name]))
    ]
    ...
    related = [
        {
            "skill": name,
            "existing": [
                alias for alias in RELATED.get(name, []) if has_term(resume_text, alias)
            ],
        }
        for name in missing
    ]
    ...
    covered = [word for word in keywords if has_term(resume_text, word)]
    years = experience_years(doc)
    role_words = {
        w.lower()
        for w in re.findall(r"\w+", text.splitlines()[0] if text.splitlines() else "")
        if w.lower() not in STOP
    }
    title = doc["personal"].get("professional_title", "")
    components = []

    def component(name, weight, score, applicable=True):
        components.append(
            {
                "name": name,
                "weight": weight,
    ...
    component(
        "Skill match", 50, len(matched) / len(sought) if sought else 0, bool(sought)
    )
    component(
        "Keyword coverage",
        20,
        len(covered) / len(keywords) if keywords else 0,
        bool(keywords),
    )
    component(
        "Experience duration",
        15,
        min(1, years / job["experience_years"]) if job["experience_years"] else 0,
        bool(job["experience_years"]),
    )
    component(
        "Education evidence",
        10,
        1
        if any(
            s["kind"] == "education" and s["visible"] and s["items"]
    ...
    component(
        "Role words",
        5,
        sum(has_term(title, w) for w in role_words) / len(role_words)
        if role_words
        else 0,
        bool(role_words),
    )
    ...
    score = (
        round(
            sum(c["weight"] * c["score"] for c in components if c["applicable"]) / total
        )
        if total
        else 0
    )
        ...
        suggestions.append(
            {
                ...
                or item.data.get("position")
                or item.data.get("statement")
                or item.data.get("title")
                or item.data.get("institution")
                or item.kind,
                "terms": terms,
                "recommended": bool(terms) or education,
                "reason": "Existing profile evidence mentions " + ", ".join(terms) + "."
                if terms
                else "Review this existing education record against the stated qualification requirement."
                if education
                else "Available in your profile. Include only if relevant to this role.",
            }
        )
    return {
        "score": score,
        ...
        "suggestions": sorted(suggestions, key=lambda s: len(s["terms"]), reverse=True),
        "disclaimer": "CareerCanvas-specific comparison, not a hiring prediction. Missing skills are gaps to review, not claims to add. Education checks presence only; experience duration does not establish seniority or relevance.",
    }
...
def run_match(resume_id: str, payload: MatchInput, db: Session = Depends(session)):
    ...
    items = db.scalars(
        select(CareerItem).where(CareerItem.profile_id == profile.id)
    ).all()
    ...
    db.add(
        Analysis(
            ...
            kind="job_match",
            result={**result, "description": payload.description},
        )
    )
    db.commit()
    return result
...
def tailor(resume_id: str, payload: TailorInput, db: Session = Depends(session)):
    if not payload.approved:
        raise HTTPException(
            422, "Review and approve the content selection before creating a copy"
        )
    source = require_resume(db, resume_id)
    profile = get_profile(db)
    items = db.scalars(
        select(CareerItem).where(
            CareerItem.profile_id == profile.id, CareerItem.id.in_(payload.selected_ids)
        )
    ).all()
    if {i.id for i in items} != set(payload.selected_ids):
        raise HTTPException(422, "Only existing profile content can be selected")
    ...
    for section in doc["sections"]:
        ...
        section["items"] = [
            ...
            if i["source_id"] is None or i["source_id"] in payload.selected_ids
        ]
        present = {i["source_id"] for i in section["items"]}
        for item in items:
            if item.kind == section["kind"] and item.id not in present:
    ...
    db.add(resume)
    db.flush()
    db.add(AuditEvent(action="Tailored Resume Copy Created", entity_id=resume.id))
    db.commit()
