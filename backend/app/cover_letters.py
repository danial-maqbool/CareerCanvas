...
class CoverVersion(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=identifier)
    cover_id: Mapped[str] = mapped_column(
        ForeignKey("cover_letters.id", ondelete="CASCADE"), index=True
    )
...
def validate_link(db, p):
    if p.resume_version_id and not db.get(ResumeVersion, p.resume_version_id):
        raise HTTPException(422, "Resume version not found")
...
def listing(db: Session = Depends(session)):
    return [
        letter_json(r)
        for r in db.scalars(select(CoverLetter).order_by(CoverLetter.updated_at.desc()))
    ]
...
def create(p: LetterInput, db: Session = Depends(session)):
    ...
    row = CoverLetter(
        ...
        resume_version_id=p.resume_version_id,
    )
    db.add(row)
    db.flush()
    db.add(AuditEvent(action="Cover Letter Created", entity_id=row.id))
    db.commit()
    return letter_json(row)
...
def edit(id: str, p: LetterInput, db: Session = Depends(session)):
    ...
    result = db.execute(
        update(CoverLetter)
        .where(CoverLetter.id == id, CoverLetter.revision == p.revision)
        .values(
            name=p.name,
            document=p.document.model_dump(),
            resume_version_id=p.resume_version_id,
    ...
    if result.rowcount != 1:
        raise HTTPException(409, "Cover letter changed. Reload before saving.")
    db.commit()
    return letter_json(require(db, id))


@router.post("/{id}/duplicate", status_code=201)
def duplicate(id: str, db: Session = Depends(session)):
    source = require(db, id)
    row = CoverLetter(
        name=source.name[:143] + " (copy)",
...
def versions(id: str, db: Session = Depends(session)):
    ...
    return [
        ...
        for v in db.scalars(
            select(CoverVersion)
            .where(CoverVersion.cover_id == id)
            .order_by(CoverVersion.created_at.desc())
        )
    ]
...
def version(id: str, p: Note, db: Session = Depends(session)):
    row = require(db, id)
    v = CoverVersion(cover_id=id, note=p.note, snapshot=letter_json(row))
    db.add(v)
    db.add(AuditEvent(action="Cover Letter Version Created", entity_id=id))
    db.commit()
    return {"id": v.id}
    ...
    if format == "docx":
        ...
        with export_slots, sync_playwright() as p:
            ...
            try:
                ...
                page.set_content(
                    "<style>@page{size:A4;margin:22mm}body{font:11pt/1.6 Arial;color:#243c33}p{white-space:pre-wrap;orphans:3;widows:3}</style>"
                    + "".join("<p>" + escape(part) + "</p>" for part in parts)
                )
                content = page.pdf(format="A4", prefer_css_page_size=True, tagged=True)
                media = "application/pdf"
            finally:
                browser.close()
    else:
        raise HTTPException(422, "Choose pdf or docx")
    db.add(AuditEvent(action="Cover Letter Exported", entity_id=id))
    db.commit()
    return Response(
        content,
        media_type=media,
        headers={
            "Content-Disposition": f"attachment; filename*=UTF-8''{quote(row.name, safe='')}.{format}",
