...
class Preferences(StrictModel):
    ...
    widgets: list[Widget] = Field(
        default_factory=lambda: [
            Widget(id=id, width=2 if id in ["pipeline", "performance"] else 1)
            for id in [
                "pipeline",
                "next",
                "interviews",
...
def preferences(db: Session = Depends(session)):
    ...
    return setting.value if setting else Preferences().model_dump()


@router.put("/preferences")
def update_preferences(p: Preferences, db: Session = Depends(session)):
    if len({w.id for w in p.widgets}) != len(p.widgets):
        raise HTTPException(422, "Dashboard widget IDs must be unique")
    setting = db.get(Setting, "workspace")
    if not setting:
        setting = Setting(key="workspace")
        db.add(setting)
    setting.value = p.model_dump()
    db.commit()
    return setting.value


@router.get("/tags")
def tags(db: Session = Depends(session)):
    return [
        {"id": t.id, "name": t.name, "color": t.color, "archived": t.archived}
        for t in db.scalars(select(Tag).order_by(Tag.name))
    ]


@router.post("/tags", status_code=201)
def create_tag(p: TagInput, db: Session = Depends(session)):
    if db.scalar(select(Tag).where(Tag.name == p.name)):
        raise HTTPException(409, "Tag name already exists")
    t = Tag(**p.model_dump())
    db.add(t)
    db.commit()
    return {"id": t.id, **p.model_dump()}
    ...
    for model in [JobApplication, Goal, CareerItem]:
        for row in db.scalars(select(model)):
            if old in row.data.get("tags", []):
                tags = [
                    new if t == old else t
                    for t in row.data["tags"]
                    if t != old or new is not None
                ]
                row.data = {**row.data, "tags": list(dict.fromkeys(tags))}
...
def edit_tag(id: str, p: TagInput, db: Session = Depends(session)):
    ...
    if db.scalar(select(Tag).where(Tag.name == p.name, Tag.id != id)):
        raise HTTPException(409, "Tag name already exists")
    if p.name != t.name:
        replace_tag(db, t.name, p.name)
    t.name = p.name
    t.color = p.color
