...
DATE_RANGE_RE = re.compile(rf"(?P<start>{DATE_TOKEN})\s*(?:-|–|—|to)\s*(?P<end>{DATE_TOKEN})", re.I)
...
def _safe_docx_zip(data: bytes) -> None:
    try:
        with zipfile.ZipFile(io.BytesIO(data)) as archive:
            ...
            if len(infos) > 5000 or sum(i.file_size for i in infos) > 32 * 1024 * 1024:
...
def _parse_docx(data: bytes) -> ParsedDocument:
    ...
    try:
        for rel in document.part.rels.values():
            if "hyperlink" in rel.reltype and rel.target_ref:
                links.add(str(rel.target_ref))
    except Exception:
        pass
    table_chars = sum(len(x) for x in table_text)
    ...
    return ParsedDocument(
        ...
        likely_multi_column=len(document.tables) > 0 and ratio > 0.45,
        scanned=False,
        warnings=warnings,
    )
...
def _parse_txt(data: bytes) -> ParsedDocument:
    ...
    return ParsedDocument(
        ...
        link_count=len(set(URL_RE.findall(text))),
    )
...
def _heading(line: str) -> str | None:
    ...
    if len(candidate) > 45:
        return None
    return ALIAS_TO_KIND.get(candidate)
...
def extract_personal(lines: list[str], full_text: str) -> dict[str, dict]:
    ...
    for url in urls:
        ...
        if key not in result:
            result[key] = {"value": url, "confidence": 0.96, "source": url}
    candidates = [
        line
    ...
    for line in lines[:15]:
        if "," in line and len(line) <= 100 and not EMAIL_RE.search(line) and not URL_RE.search(line):
            ...
            if len(parts) == 2 and not any(ch.isdigit() for ch in line):
...
def _date_parts(line: str) -> tuple[str, str, bool]:
    ...
    return start, "" if end.lower() in {"present", "current", "now"} else end, end.lower() in {"present", "current", "now"}
...
def parse_experience(lines: list[str]) -> list[dict]:
    results: list[dict] = []
    date_indices = [i for i, line in enumerate(lines) if DATE_RANGE_RE.search(line)]
    for pos, i in enumerate(date_indices):
        date_line = lines[i]
        start, end, current = _date_parts(date_line)
        inline = DATE_RANGE_RE.sub("", date_line).strip(" |–—-")
        header = inline or (lines[i - 1] if i > 0 else "")
        role, company = _split_header(header)
        if not company and i > 1 and lines[i - 2] != header:
            company = lines[i - 2]
        if not role or not company:
            continue
        next_i = date_indices[pos + 1] if pos + 1 < len(date_indices) else len(lines)
        bullet_start = i + 1
        bullets = [
            {"id": str(uuid4()), "text": line}
            for line in lines[bullet_start:next_i]
            if len(line) > 8 and not DATE_RANGE_RE.search(line)
        ][:12]
        evidence = " | ".join(lines[max(0, i - 2) : min(len(lines), i + 3)])[:1000]
        results.append(
            {
                "data": {
                    "company": company[:200],
...
def parse_education(lines: list[str]) -> list[dict]:
    ...
    for index, i in enumerate(date_indices):
        end_at = date_indices[index + 1] if index + 1 < len(date_indices) else len(lines)
        before = lines[max(0, i - 2) : i] if i < len(lines) else lines[:2]
        if not before:
            continue
        degree = before[-1]
        institution = before[-2] if len(before) > 1 else ""
        if not institution or not degree:
            continue
        start, end, _ = _date_parts(lines[i]) if i < len(lines) else ("", "", False)
        rest = lines[i + 1 : end_at] if i < len(lines) else lines[2:]
        gpa = next((m.group(1) for line in rest if (m := re.search(r"GPA\s*[: ]\s*([\d.]+(?:\s*/\s*[\d.]+)?)", line, re.I))), "")
        results.append(
            {
                "data": {
                ...
                "confidence": 0.66,
                "source": " | ".join(before + ([lines[i]] if i < len(lines) else []))[:1000],
            }
        )
    return results
...
def parse_skills(lines: list[str]) -> list[dict]:
    ...
    for line in lines:
        ...
        for value in re.split(r"[,;|•·]", line):
            value = value.strip(" .")
            key = _norm(value)
            if not value or len(value) > 80 or key in seen or len(key) < 2:
                continue
...
def parse_projects(lines: list[str]) -> list[dict]:
    ...
    for line in lines:
        if URL_RE.fullmatch(line):
            ...
            continue
        is_title = len(line) <= 100 and not line.endswith(".") and not re.match(r"^(built|developed|created|implemented|designed|led|used)\b", line, re.I)
        if is_title:
            if current:
...
def parse_portfolio(lines: list[str]) -> list[dict]:
    ...
    for line in lines:
        ...
        if urls:
            for url in urls:
                without = line.replace(url, "").strip(" |–—-:")
                title = without or pending_title or url
                results.append(
                    {
                        "data": {
                            "title": title[:200],
...
def _simple_line_items(lines: list[str], kind: str) -> list[dict]:
    ...
    for line in lines:
        ...
        if kind == "certifications":
            ...
            data = {"name": left[:200], "issuer": right[:200], "date": "", "expiry": "", "credential_id": "", "credential_url": ""}
        elif kind == "achievements":
            data = {"statement": line[:3000], "company": "", "project": "", "skills": [], "metric": "", "category": "Impact", "tags": []}
        elif kind == "publications":
            doi = DOI_RE.search(line)
            data = {"title": line[:500], "authors": "", "venue": "", "year": next(iter(re.findall(r"\b(?:19|20)\d{2}\b", line)), ""), "doi": doi.group(0) if doi else "", "url": next(iter(URL_RE.findall(line)), ""), "citation": line[:2000], "description": ""}
        elif kind == "languages":
...
def _duplicate_score(kind: str, incoming: dict, existing: CareerItem) -> float:
    ...
    if not keys:
        return 0.0
    matches = sum(
        _norm(str(incoming.get(k, ""))) == _norm(str(data.get(k, "")))
        and bool(_norm(str(incoming.get(k, ""))))
        for k in keys
    )
...
def _merge(existing: dict, incoming: dict) -> dict:
    ...
    for key, value in incoming.items():
        ...
        if isinstance(value, list):
            ...
            seen = {str(v).lower() for v in combined}
            for item in value:
...
def preview(payload: UploadPayload, db: Session = Depends(session)):
    data = _safe_decode(payload)
    parsed = parse_document(payload.filename, data)
