...
class Story(RecordMixin, Base):
    __tablename__ = "star_stories"


class Question(RecordMixin, Base):
...
class InterviewData(StrictModel):
    ...
    checklist: list[Task] = Field(
        default_factory=lambda: [Task(text=t) for t in CHECKLIST], max_length=100
    )
...
MODELS = {
    "goals": (Goal, GoalData),
    "interviews": (Interview, InterviewData),
    "contacts": (Contact, ContactData),
    "stories": (Story, StoryData),
...
def validate_record(db, kind, p):
    ...
    try:
        data = schema.model_validate(p.data).model_dump()
    except ValueError as e:
        raise HTTPException(422, str(e))
    if p.application_id and (
        kind not in ["interviews", "contacts"]
        or not db.get(JobApplication, p.application_id)
    ):
        raise HTTPException(422, "Linked application not found")
    if kind == "stories":
        for field, item_kind in [
            ...
            for id in data[field]:
                ...
                if not item or item.kind != item_kind:
                    raise HTTPException(422, "Linked profile content not found")
    if kind == "interviews":
        for field, model in [("story_ids", Story), ("question_ids", Question)]:
            if any(not db.get(model, id) for id in data[field]):
                raise HTTPException(422, "Linked preparation record not found")
    return data
...
def listing(kind: str, db: Session = Depends(session)):
    ...
    return [
        record_json(r)
        for r in db.scalars(select(model).order_by(model.updated_at.desc()))
    ]
...
def create(kind: str, p: RecordInput, db: Session = Depends(session)):
    model, _ = model_for(kind)
    data = validate_record(db, kind, p)
    row = model(title=p.title, data=data)
    if hasattr(model, "application_id"):
        row.application_id = p.application_id
    db.add(row)
    db.flush()
    db.add(AuditEvent(action=f"{kind.title()} Added", entity_id=row.id))
    db.commit()
...
def edit(kind: str, id: str, p: RecordInput, db: Session = Depends(session)):
    ...
    if hasattr(model, "application_id"):
        values["application_id"] = p.application_id
    result = db.execute(
        update(model)
        .where(model.id == id, model.revision == p.revision)
        .values(**values)
    )
    if result.rowcount != 1:
        raise HTTPException(409, "Record changed; reload before saving")
    db.commit()
    return record_json(db.get(model, id))
...
def remove(kind: str, id: str, db: Session = Depends(session)):
    ...
    if kind in ["stories", "questions"]:
        ...
        for interview in db.scalars(select(Interview)):
            if id in interview.data.get(field, []):
                interview.data = {
                    **interview.data,
                    field: [x for x in interview.data[field] if x != id],
                }
