...
def make_backup(db):
    ...
    for model in MODELS_ORDER:
        ...
        for row in db.scalars(select(model)):
            if model == Setting and row.key != "workspace":
                continue
            rows.append(
                {
                    c.name: iso(getattr(row, c.name))
...
def validate_backup(backup):
    ...
    if sum(len(r) for r in backup.tables.values()) > 100000:
            ...
            for key, col in columns.items():
                ...
                if isinstance(col.type, DateTime):
                    ...
                    if (
                        ...
                        or col.type.length
                        and len(value) > col.type.length
                    ):
                        raise ValueError("Invalid text length")
            pk = "key" if model == Setting else "id"
            if not row[pk] or row[pk] in seen:
                raise ValueError(f"Duplicate or missing IDs in {name}")
            seen.add(row[pk])
            if model == CareerProfile:
                row["personal"] = PersonalDetails.model_validate(
                    row["personal"]
                ).model_dump()
            if model == CareerItem:
                row["data"] = ItemInput(kind=row["kind"], data=row["data"]).data
            if model == Resume:
                row["document"] = ResumeDocument.model_validate(
                    row["document"]
                ).model_dump()
            if model == ResumeVersion:
                ResumeDocument.model_validate(row["snapshot"]["document"])
            if model == CoverLetter:
                row["document"] = LetterDocument.model_validate(
                    row["document"]
                ).model_dump()
            if model == CoverVersion:
                LetterDocument.model_validate(row["snapshot"]["document"])
            if model == JobApplication:
                row["data"] = JobData.model_validate(row["data"]).model_dump()
            for _, (record, schema) in MODELS.items():
                if model == record:
                    row["data"] = schema.model_validate(row["data"]).model_dump()
            if model == Tag:
                TagInput.model_validate(
                    {k: row[k] for k in ["name", "color", "archived"]}
                )
            if model == Setting:
                if row["key"] != "workspace":
                    raise ValueError(
                        "Only non-secret workspace preferences may be restored"
                    )
    ...
    for name, model in TABLES.items():
        for column in model.__table__.columns:
            for fk in column.foreign_keys:
                ...
                key = fk.column.name
                ids = {r[key] for r in prepared[target]}
                if any(
                    r[column.name] is not None and r[column.name] not in ids
                    for r in prepared[name]
                ):
                    raise ValueError(f"Broken relationship in {name}.{column.name}")
    if len(prepared["career_profiles"]) > 1:
        raise ValueError("This version supports one career profile")
    return prepared
...
@router.get("/backup")
def backup(db: Session = Depends(session)):
    return Response(
        json.dumps(make_backup(db), ensure_ascii=False, indent=2),
        media_type="application/json",
        headers={
            "Content-Disposition": 'attachment; filename="CareerCanvas-backup.json"',
...
def preview(p: Backup):
    try:
        rows = validate_backup(p)
    except (ValueError, KeyError, TypeError) as e:
        raise HTTPException(422, "Invalid backup: " + str(e)) from e
...
def restore(p: RestoreInput, db: Session = Depends(session)):
    if not p.approved:
        raise HTTPException(
            422,
            "Review the restore preview and explicitly approve replacing the workspace",
        )
    try:
        rows = validate_backup(p.backup)
    except (ValueError, KeyError, TypeError) as e:
        raise HTTPException(422, "Invalid backup: " + str(e)) from e
    safety = ROOT / "data" / "backups"
    safety.mkdir(parents=True, exist_ok=True)
    path = safety / (
        "before-restore-" + utcnow().strftime("%Y%m%d-%H%M%S-%f") + ".json"
    )
    path.write_text(
        json.dumps(make_backup(db), ensure_ascii=False, indent=2), encoding="utf-8"
    )
    try:
        ...
        for model in MODELS_ORDER:
            if rows[model.__tablename__]:
                db.execute(model.__table__.insert(), rows[model.__tablename__])
        db.add(AuditEvent(action="Workspace Restored"))
        db.commit()
    except IntegrityError as e:
        db.rollback()
        raise HTTPException(
            422,
            "Backup constraints were invalid. The existing workspace was preserved.",
        ) from e
    return {
        "restored": True,
...
def import_resume(p: ResumeImport, db: Session = Depends(session)):
    row = Resume(
        ...
        document=p.document.model_dump(),
    )
    db.add(row)
    db.flush()
    db.add(AuditEvent(action="Resume Imported", entity_id=row.id))
    db.commit()
