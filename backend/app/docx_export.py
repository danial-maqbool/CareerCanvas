...
def render_docx(data: dict):
    ...
    style = data["style"]
    hidden = set(personal.get("hidden_fields", []))
    ...
    normal.font.name = "Arial" if style["font"] == "System Sans" else style["font"]
    normal.font.size = Pt(style["font_size"])
    normal.font.color.rgb = RGBColor.from_string(style["text"][1:])
    ...
    if personal.get("email") and "email" not in hidden:
        hyperlink(
            contact,
            personal["email"],
    ...
    for key in ["phone", "city", "country"]:
        if personal.get(key) and key not in hidden:
            contact.add_run("  |  " + personal[key])
    for key in ["linkedin", "github", "portfolio", "website"]:
        if personal.get(key) and key not in hidden:
            contact.add_run("  |  ")
            hyperlink(contact, key.title(), personal[key], style["accent"][1:])
    if personal.get("summary") and "summary" not in hidden:
        document.add_heading("Professional Summary", level=1)
        if data.get("summary_rich"):
            rich_docx(document, data["summary_rich"])
        else:
    ...
    for sec in data["sections"]:
        ...
        if sec["kind"] == "skills":
            document.add_paragraph(
                sec.get("separator", " · ").join(
                    item["data"]["name"] for item in sec["items"]
                )
            )
            continue
        for item in sec["items"]:
            ...
            if start or end:
                p.add_run("  |  " + start + (" - " + end if end else "")).italic = True
            details = " · ".join(
                str(d[k])
                for k in ["company", "issuer", "venue", "role", "location"]
                if d.get(k)
            )
            if details:
            ...
            if d.get("degree"):
                document.add_paragraph(
                    " · ".join(
                        str(d[k]) for k in ["degree", "field", "gpa"] if d.get(k)
                    )
                )
            for key in ["authors", "description", "citation", "proficiency"]:
                if d.get(key):
                    document.add_paragraph(str(d[key]))
            for bullet in d.get("bullets", []):
                p = document.add_paragraph(bullet["text"], style="List Bullet")
                p.paragraph_format.space_after = Pt(style["bullet_spacing"] * 0.75)
            for key in ["technologies", "courses", "honors"]:
                if d.get(key):
                    document.add_paragraph(key.title() + ": " + ", ".join(d[key]))
            for key in ["url", "github", "credential_url"]:
                if d.get(key):
                    hyperlink(
                        document.add_paragraph(), d[key], d[key], style["accent"][1:]
                    )
            if d.get("doi"):
                document.add_paragraph("DOI: " + d["doi"])
            if d.get("credential_id"):
                document.add_paragraph("Credential ID: " + d["credential_id"])
            if sec["kind"] == "references":
                if d.get("email"):
                    hyperlink(
                        document.add_paragraph(), d["email"], "mailto:" + d["email"]
                    )
                if d.get("phone"):
                    document.add_paragraph(d["phone"])
            ...
            if item_paragraphs:
                item_paragraphs[-1].paragraph_format.keep_with_next = False
...
def rich_docx(document, node, paragraph=None, bullet=False):
    ...
    if kind == "text":
        ...
        for mark in node.get("marks") or []:
            ...
            if mark["type"] == "italic":
                run.italic = True
    elif kind == "hardBreak" and paragraph is not None:
        ...
        for child in node.get("content") or []:
            rich_docx(document, child, paragraph, bullet or kind == "bulletList")
