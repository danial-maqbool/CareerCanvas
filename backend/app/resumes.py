...
def serialize(resume):
    analyses = sorted(
        [
            a
            for a in getattr(resume, "analyses", [])
            if a.kind == "ats" and a.revision == resume.revision
        ],
        key=lambda a: iso(a.created_at),
        reverse=True,
    )
    latest = analyses[0].result if analyses else {}
    return {
        key: getattr(resume, key)
        for key in [
            "id",
            "name",
            "purpose",
...
def list_resumes(db: Session = Depends(session)):
    return [
        serialize(r)
        for r in db.scalars(select(Resume).order_by(Resume.updated_at.desc())).all()
    ]
...
def create_resume(payload: CreateResume, db: Session = Depends(session)):
    ...
    if {i.id for i in items} != set(payload.selected_ids):
        raise HTTPException(
            422, "Selected content was not found in this career profile"
        )
    sections = []
    for kind, heading in HEADINGS.items():
        selected = [
            ResumeItem(kind=i.kind, source_id=i.id, data=deepcopy(i.data))
            for i in items
            if i.kind == kind
        ]
        sections.append(
            ResumeSection(
                kind=kind, heading=heading, visible=bool(selected), items=selected
            )
        )
    doc = ResumeDocument(
        personal=profile.personal, sections=sections, template=payload.template
    )
    resume = Resume(
...
def save_resume(resume_id: str, payload: UpdateResume, db: Session = Depends(session)):
    ...
    old_name, old_template, old_archived = (
        resume.name,
    ...
    if result.rowcount != 1:
        raise HTTPException(
            409,
            "This resume changed in another window. Reload before saving to avoid overwriting newer edits.",
        )
    for changed, action in [
        (old_name != payload.name, "Resume Renamed"),
        (old_template != payload.document.template, "Template Changed"),
        (
            old_archived != payload.archived,
            "Resume Archived" if payload.archived else "Resume Unarchived",
        ),
    ]:
        if changed:
