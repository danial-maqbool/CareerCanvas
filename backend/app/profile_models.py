...
class CareerProfile(Base):
    ...
    items: Mapped[list['CareerItem']] = relationship(back_populates='profile', cascade='all, delete-orphan')


class CareerItem(Base):
    __tablename__ = 'career_items'
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=identifier)
    profile_id: Mapped[str] = mapped_column(ForeignKey('career_profiles.id', ondelete='CASCADE'), index=True)
