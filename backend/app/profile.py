...
def completion(profile, items):
    checks = {
        ...
        'Add experience or a project': any(i.kind in ('experience', 'projects') for i in items),
        'Add your skills': any(i.kind == 'skills' for i in items),
    }
    return {'score': round(sum(checks.values()) / len(checks) * 100), 'suggestions': [k for k, v in checks.items() if not v]}


@router.get('')
def read_profile(db: Session = Depends(session)):
    profile = get_profile(db)
    items = db.scalars(select(CareerItem).where(CareerItem.profile_id == profile.id).order_by(CareerItem.created_at)).all()
    return {'id': profile.id, 'personal': profile.personal, 'items': [serialize_item(i) for i in items], 'completion': completion(profile, items)}
