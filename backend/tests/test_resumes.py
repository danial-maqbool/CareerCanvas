def make_resume(client):
    ...
    response = client.post('/api/resumes', json={'name':'AI Engineer Resume','target_role':'AI Engineer','selected_ids':[i['id'] for i in profile['items'] if i['kind'] != 'references']})
    assert response.status_code == 201, response.text
    return response.json()


def update_payload(resume):
    return {key: resume[key] for key in ['name','revision','target_role','archived','document']}
...
def test_independent_document_duplicate_and_archive(client):
    ...
    assert client.get(f'/api/resumes/{original["id"]}').json()['document'] == original['document']
...
def test_optimistic_save_and_template_preservation(client):
    ...
    for template in ['Classic','Technical','Modern']:
        ...
        assert client.put(f'/api/resumes/{resume["id"]}',json=payload).status_code == 409
        resume = saved.json()
...
def test_profile_deletion_does_not_remove_resume_content(client):
    ...
    assert client.get(f'/api/resumes/{resume["id"]}').json()['document'] == resume['document']


def test_missing_selection_and_style_safety(client):
    assert client.post('/api/resumes',json={'name':'Bad','selected_ids':['missing']}).status_code == 422
    resume = make_resume(client)
    payload = update_payload(resume)
