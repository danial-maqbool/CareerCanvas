...
def test_profile_personal_persists(client):
    ...
    personal = {**profile['personal'], 'full_name': 'Test Person', 'email': 'test@example.com'}
...
def test_typed_library_crud(client, item):
    ...
    assert client.put(f'/api/profile/items/{record["id"]}', json={'kind': record['kind'], 'data': record['data']}).status_code == 200
...
def test_invalid_career_items_rejected(client):
    assert client.post('/api/profile/items', json={'kind':'skills','data':{'name':''}}).status_code == 422
    assert client.post('/api/profile/items', json={'kind':'experience','data':{'company':'Test'}}).status_code == 422
    assert client.post('/api/profile/items', json={'kind':'skills','data':{'name':'Python','unknown':True}}).status_code == 422
    assert client.put('/api/profile/personal', json={'website':'javascript:alert(1)'}).status_code == 422
