def test_observed_analytics_and_goals(client):
    ...
    a, b, c = jobs
    for job, stages in [
        (a, ["Applied", "Interview", "Offer", "Rejected"]),
        (b, ["Applied"]),
    ]:
        for stage in stages:
            job = client.patch(
                "/api/applications/" + job["id"] + "/stage",
                json={"status": stage, "revision": job["revision"]},
            ).json()
    data = client.get("/api/analytics").json()
    assert (
        data["sent"] == 2
        and data["responses"] == 1
        and data["interviews"] == 1
        and data["offers"] == 1
    )
    ...
    goal = client.post(
        ...
        json={
            ...
            "data": {
                "milestones": [
                    {"text": "Ship a project", "done": True},
                    {"text": "Write case study", "done": False},
                ]
            },
        },
    )
    ...
    assert (
        client.post(
            "/api/career/goals", json={"title": "Invalid", "data": {"progress": 101}}
        ).status_code
        == 422
    )
