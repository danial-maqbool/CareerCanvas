def test_backup_roundtrip_and_rejection(client):
    ...
    resume = client.post(
        ...
        json={
            "name": "Backup resume",
            "selected_ids": [i["id"] for i in profile["items"]],
        },
    ).json()
    client.post(
        "/api/applications",
        json={"company": "Backup Cedar", "role": "Engineer", "resume_id": resume["id"]},
    )
    backup = client.get("/api/workspace/backup").json()
    assert (
        backup["format"] == "CareerCanvas Backup"
        and "api_key" not in str(backup).lower()
    )
    assert client.post("/api/workspace/restore/preview", json=backup).status_code == 200
    assert (
        client.post("/api/workspace/restore", json={"backup": backup}).status_code
        == 422
    )
    client.post("/api/career/goals", json={"title": "Temporary test goal"})
    restored = client.post(
        "/api/workspace/restore", json={"backup": backup, "approved": True}
    )
    ...
    bad["tables"]["job_applications"][0]["resume_id"] = "missing"
    assert (
        client.post(
            "/api/workspace/restore", json={"backup": bad, "approved": True}
        ).status_code
        == 422
    )
...
def test_tags_delete_keeps_records_and_preferences(client):
    ...
    client.post(
        "/api/applications",
        json={"company": "Keep me", "role": "Engineer", "data": {"tags": ["Remote"]}},
    )
    assert (
        client.put(
            "/api/tags/" + tag["id"], json={"name": "Flexible", "color": "#123456"}
        ).status_code
        == 200
    )
    ...
    p = client.get("/api/preferences").json()
    p["theme"] = "Dark"
    p["widgets"].reverse()
    p["widgets"][0]["visible"] = False
