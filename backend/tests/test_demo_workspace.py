def test_complete_demo_workspace(client):
    response = client.post("/api/demo/workspace")
    ...
    assert len({r["document"]["template"] for r in resumes}) == 4
    assert any(
        len(client.get("/api/resumes/" + r["id"] + "/versions").json()) == 2
        for r in resumes
    )
    analytics = client.get("/api/analytics").json()
    assert analytics["sent"] == 10 and analytics["offers"] == 2
