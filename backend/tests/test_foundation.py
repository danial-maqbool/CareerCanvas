...
def test_migration_is_repeatable_and_preserves_settings(tmp_path):
    ...
    with engine.begin() as connection:
        connection.execute(text("INSERT INTO settings VALUES ('theme', '{\"mode\":\"dark\"}')"))
    command.upgrade(config, 'head')
...
def test_local_health_and_unknown_api(tmp_path):
    ...
    with TestClient(app) as client:
        assert client.get('/api/health').json() == {'status': 'ok', 'storage': 'local', 'ai_enabled': False}
        assert client.get('/api/not-real').status_code == 404
        assert client.post('/api/test', headers={'origin': 'https://example.com'}).status_code == 403
