...
def test_job_extraction_categorizes_requirements_and_synonyms():
    ...
    assert extract_job("Required: K8s and Postgres")["required_skills"] == [
        "Kubernetes",
        "PostgreSQL",
    ]
...
def test_match_and_reviewed_tailoring_never_add_missing_skills(client):
    r = make_resume(client)
    path = f"/api/resumes/{r['id']}"
    ...
    payload = {
        "description": DEMO_JOB,
        "selected_ids": selected,
        "name": "Tailored AI Engineer",
    }
    ...
    tailored = result.json()
    assert tailored["id"] != r["id"]
