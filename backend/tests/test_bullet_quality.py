...
def test_weak_and_evidence_based_bullets():
    ...
    strong = assess_bullet(
        "Developed 14 REST endpoints used by three internal services."
    )
    assert weak["score"] == 0
    assert strong["score"] == 100
    assert not assess_bullet("Built a reliable document parser for the research team.")[
        "checks"
    ][3]["passed"]
