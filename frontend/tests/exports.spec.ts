...
test('editor downloads PDF and JSON after saving pending edits',async({page,request})=>{
  ...
  const r=await (await request.post('/api/resumes',{data:{name:`Export ${Date.now()}`,selected_ids:p.items.filter((i:{kind:string})=>['experience','education','skills','projects'].includes(i.kind)).map((i:{id:string})=>i.id)}})).json()
