...
test('pointer section drag and keyboard bullet drag persist after reload',async({page,request})=>{
  const profile=await (await request.get('/api/profile')).json()
  const resume=await (await request.post('/api/resumes',{data:{name:`Drag test ${Date.now()}`,selected_ids:profile.items.map((i:{id:string})=>i.id)}})).json()
  await page.goto('/')
  ...
  const projects=await page.getByRole('button',{name:'Drag Projects section',exact:true}).boundingBox()
  const experience=await page.getByRole('button',{name:'Drag Experience section',exact:true}).boundingBox()
  ...
  await page.mouse.up()
  await expect.poll(async()=>{const r=await (await request.get(`/api/resumes/${resume.id}`)).json();return r.document.sections[0].kind}).toBe('projects')
  ...
  const secondId=resume.document.sections.find((s:{kind:string})=>s.kind==='experience').items[0].data.bullets[1].id
  await expect(page.locator('.drawer [role="status"]').last()).toContainText(`over droppable area ${secondId}`)
  await page.keyboard.press('Space')
  ...
  expect(saved.document.sections.find((s:{kind:string})=>s.kind==='experience').items[0].data.bullets[0].text).toBe(second)
  await page.reload()
  await page.getByRole('button',{name:'Resumes',exact:true}).click()
  await page.getByRole('button',{name:`Open ${resume.name}`,exact:true}).click()
