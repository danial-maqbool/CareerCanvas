...
test('fixing a missing email increases the transparent ATS score by 15', async ({
  page,
  request,
}) => {
  ...
  await expect(
    page.locator('.ats-check').filter({ hasText: 'Contact email present' })
  ).toContainText('HIGH')
  ...
  await page.screenshot({
    path: 'test-results/ats-analysis.png',
    fullPage: true,
  })
})
