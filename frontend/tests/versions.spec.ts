...
test('create compare and restore versions without deleting newer snapshots', async ({
  page,
  request,
}) => {
  ...
  await page
    ...
    .fill('A shorter version of my experience.')
  await page.getByRole('button', { name: 'Versions', exact: true }).click()
  ...
  await expect(page.locator('.diff-card').filter({ hasText: 'personal / summary' })).toContainText(
    'A shorter version of my experience.'
  )
  await page.screenshot({
    path: 'test-results/version-comparison.png',
    fullPage: true,
  })
  await page
