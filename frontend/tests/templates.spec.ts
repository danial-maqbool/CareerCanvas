...
test('all templates preview real content and switching preserves the document',async({page,request})=>{
  const profile=await (await request.get('/api/profile')).json()
  const resume=await (await request.post('/api/resumes',{data:{name:`Templates ${Date.now()}`,selected_ids:profile.items.map((i:{id:string})=>i.id)}})).json()
  await page.goto('/')
  ...
  for(const template of ['Classic','Technical','Modern']){
    ...
    await page.getByRole('button',{name:`Preview ${template} with my data`,exact:true}).click()
    await expect(page.locator('dialog').last().locator('.resume-paper')).toContainText('Alex Morgan')
    await page.getByRole('button',{name:`Apply ${template}`,exact:true}).click()
    await expect.poll(async()=>{const r=await (await request.get(`/api/resumes/${resume.id}`)).json();return r.document.template}).toBe(template)
  }
  ...
  await page.screenshot({path:'test-results/template-gallery.png',fullPage:true})
})
