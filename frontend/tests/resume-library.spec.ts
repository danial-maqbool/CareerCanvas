...
test('create, name, duplicate, rename, archive and delete independent resumes', async ({page,request}) => {
  ...
  const copy = page.locator('.resume-card').filter({has:page.getByRole('heading',{name:`${name} (copy)`,exact:true})})
  ...
  const renamed = page.locator('.resume-card').filter({has:page.getByRole('heading',{name:`${name} Research`,exact:true})})
