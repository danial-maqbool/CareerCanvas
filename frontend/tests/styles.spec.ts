...
test('typography, colors and Letter sizing update immediately and persist', async ({
  page,
  request,
}) => {
  ...
  await expect(page.locator('.resume-paper').first()).toHaveCSS('font-family', 'Georgia')
  await expect(page.locator('.resume-paper').first()).toHaveCSS('font-size', '16px')
  await expect(page.locator('.resume-paper h1')).toHaveCSS('color', 'rgb(40, 79, 128)')
  await expect(page.locator('.paginated-document')).toContainText('June 2023')
  ...
  expect(saved.document.style).toMatchObject({
    font: 'Georgia',
    page_size: 'Letter',
    font_size: 12,
    accent: '#284f80',
