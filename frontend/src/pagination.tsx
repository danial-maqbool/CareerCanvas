...
export function paginateDocument(document: ResumeDocument): Pagination {
  ...
  sandbox.style.cssText =
    'position:fixed;left:-30000px;top:0;visibility:hidden;pointer-events:none;'
  window.document.body.appendChild(sandbox)
  const warnings: string[] = [],
    pages: PageLayout[] = []
  let current: PageLayout = {
  ...
  function nextPage() {
    ...
    current = { document: { ...document, sections: [] }, first: false }
  }
  function add(section: ResumeSection, item: ResumeItem, depth = 0) {
    ...
    if (!existing) {
      ...
      current.document.sections.push(existing)
    }
    existing.items.push(item)
    if (fits(current)) return
    existing.items.pop()
    if (!existing.items.length)
      current.document.sections = current.document.sections.filter((s) => s !== existing)
    if (hasContent()) {
      nextPage()
      add(section, item, depth)
      return
    }
    const bullets = item.data.bullets as { id: string; text: string }[] | undefined
    if (depth < 12 && bullets && bullets.length > 1) {
      warnings.push(
        `${section.heading}: a long item continues across pages. Reduce spacing, move the section, or shorten the content if you prefer to keep it together.`
      )
      ...
      add(
        section,
        {
          ...item,
          id: `${item.id}-a`,
          data: { ...item.data, bullets: bullets.slice(0, split) },
        },
        depth + 1
      )
      add(
    ...
    if (
      ...
      item.data.description.length > 600
    ) {
      const words = item.data.description.split(' '),
        split = Math.ceil(words.length / 2)
      add(
        section,
        {
          ...item,
          ...
          data: {
            ...item.data,
            description: words.slice(0, split).join(' '),
            bullets: [],
          },
        },
        depth + 1
      ...
      add(
        section,
        {
          ...item,
          id: `${item.id}-b`,
          data: { ...item.data, description: words.slice(split).join(' ') },
        },
        depth + 1
      )
      return
    ...
    warnings.push(
      `${section.heading}: an item is taller than the usable page. Shorten it, reduce spacing, or split the content.`
    )
    nextPage()
  }
  try {
    if (!fits(current))
      warnings.push(
        'The header or summary exceeds one page. Shorten the summary or reduce spacing.'
      )
    const visible = document.sections.filter((s) => s.visible)
    // The dedicated side rail must participate before main-column pagination.
    const ordered =
      document.template === 'Two Column'
        ? [
            ...visible.filter((s) => ['skills', 'certifications', 'languages'].includes(s.kind)),
            ...visible.filter((s) => !['skills', 'certifications', 'languages'].includes(s.kind)),
          ]
        : visible
    for (const section of ordered) {
      if (!section.items.length) {
        ...
        current.document.sections.push(candidate)
        if (!fits(current) && hasContent()) {
          current.document.sections.pop()
          nextPage()
          current.document.sections.push(candidate)
        }
      }
    ...
    if (hasContent()) pages.push(current)
    return {
      pages: pages.length ? pages : [{ document, first: true }],
