...
export type RichNode = {type:string,text?:string,content?:RichNode[],marks?:{type:string}[]}
export type ResumeDocument = {summary_rich?:RichNode|null, personal:Personal, sections:ResumeSection[], template:string, style:ResumeStyle }
export type Resume = {open_panel?:string,id:string, name:string, purpose:string, target_role:string, document:ResumeDocument, archived:boolean, primary:boolean, revision:number, updated_at:string, created_at:string,ats_score?:number|null,page_count?:number|null}
