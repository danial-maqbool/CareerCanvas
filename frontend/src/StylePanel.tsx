...
export default function StylePanel() {
  ...
  return (
    <>
      <p className="form-note">Make it yours. Every adjustment updates the paper immediately.</p>
      ...
      {[
        ...
        <label className="field range-field" key={control.key}>
          <span>
            {control.label}
            <output>
              {style[control.key as keyof ResumeStyle]} {control.unit}
            </output>
          </span>
          <input
            ...
            max={control.max}
            step={control.step}
            value={style[control.key as keyof ResumeStyle]}
            onChange={(e) => set(control.key as 'font_size', Number(e.target.value))}
          />
        </label>
      ))}
      ...
      <label className="field">
        ...
        <select value={style.heading_style} onChange={(e) => set('heading_style', e.target.value)}>
      ...
      <label className="field">
        ...
        <select value={style.date_style} onChange={(e) => set('date_style', e.target.value)}>
      ...
      <p className="form-note">
        System fonts only. If a font is unavailable on this computer, the system uses its local
        fallback. Minimum font size is 9 pt.
      </p>
    </>
  )
}
