...
export function ItemEditor({ kind, item, onClose, onSaved }: { kind: Kind, item?: CareerItem, onClose: () => void, onSaved: () => void }) {
  ...
  const submit = handleSubmit(async values => {
    ...
    try { await api(`/profile/items${item ? `/${item.id}` : ''}`, json(item ? 'PUT' : 'POST', { kind, data })); onSaved(); onClose() } catch (e) { setError((e as Error).message) }
  })
  return <Drawer title={`${item ? 'Edit' : 'Add'} ${labels[kind]}`} onClose={onClose}><form onSubmit={submit} className="drawer-form"><p className="form-note">Saved to your reusable Career Profile. Existing resume copies stay independent.</p>{fields[kind].map(field => <label className={`field ${field.type === 'check' ? 'checkbox-field' : ''}`} key={field.key}><span>{field.label}{field.required && ' *'}</span>{field.type === 'check' ? <input type="checkbox" {...register(field.key)}/> : field.type === 'select' ? <select {...register(field.key)}>{field.options!.map(option => <option key={option}>{option}</option>)}</select> : field.type === 'long' || field.type === 'bullets' ? <textarea rows={field.type === 'bullets' ? 5 : 3} {...register(field.key)}/> : <input {...register(field.key)}/>} {errors[field.key] && <small className="error">{String(errors[field.key]?.message)}</small>}</label>)}{error && <p role="alert" className="error">{error}</p>}<div className="drawer-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={isSubmitting}>{isSubmitting ? 'Saving…' : 'Save to profile'}</button></div></form></Drawer>
}
