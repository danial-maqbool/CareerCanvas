...
export default function Profile({ initialKind }: { initialKind?: Kind }) {
  ...
  const [search,setSearch]=useState(''),[category,setCategory]=useState('All'),[github,setGithub]=useState(false)
  ...
  const visible = profile.items.filter(item => item.kind === tab && JSON.stringify(item.data).toLowerCase().includes(search.toLowerCase()) && (category==='All'||item.data.category===category||item.data.kind===category))
  return <>
    ...
    {!initialKind && <div className="tabs" role="tablist" aria-label="Career profile sections">{(['about', ...Object.keys(labels)] as (Kind | 'about')[]).map(kind => <button role="tab" aria-selected={tab === kind} key={kind} onClick={() => {setTab(kind);setCategory('All');setSearch('')}}>{kind === 'about' ? 'About' : labels[kind]}{kind !== 'about' && <span>{profile.items.filter(i => i.kind === kind).length}</span>}</button>)}</div>}
    {notice && <p className="notice" role="status"><Check size={14}/>{notice}</p>}{error && <p role="alert" className="error">{error}</p>}
    {tab === 'about' ? <About key={profile.id} personal={profile.personal} onSaved={refresh}/> : <><div className="section-toolbar"><div><h2>{labels[tab]} <span className="count-badge">{visible.length}</span></h2><p>{tab === 'references' ? 'Private by default. Include references in a resume only when explicitly selected.' : 'A growing library of what you bring to the table.'}</p></div><button className="button primary" onClick={() => setEditing({kind:tab})}><Plus size={15}/> Add {labels[tab]}</button></div><div className="library-toolbar"><input className="library-search" aria-label="Search career content" placeholder="Find a skill, project or achievement…" value={search} onChange={e=>setSearch(e.target.value)}/>{['skills','portfolio'].includes(tab)&&<select aria-label="Filter category" value={category} onChange={e=>setCategory(e.target.value)}><option>All</option>{Array.from(new Set(profile.items.filter(i=>i.kind===tab).map(i=>String(i.data.category||i.data.kind||'Other')))).map(c=><option key={c}>{c}</option>)}</select>}{tab==='projects'&&<button className="button secondary" onClick={()=>setGithub(true)}>Import public GitHub project</button>}</div><div className={tab === 'skills' ? 'skills-grid' : 'content-grid'}>{visible.map(item => <article className={`panel content-card kind-${item.kind}`} key={item.id}>{item.kind==='portfolio'&&<div className={`portfolio-art art-${String(item.data.kind||'Project').toLowerCase().replaceAll(' ','-')}`}><span>{String(item.data.kind||'Project')}</span><strong>{itemTitle(item)}</strong><div className="portfolio-art-lines"><i/><i/><i/><i/></div>{item.data.thumbnail&&<a href={String(item.data.thumbnail)} target="_blank" rel="noreferrer">Open thumbnail ↗</a>}</div>}<div className="card-heading"><span className="item-icon">{item.kind === 'education' ? <GraduationCap size={20}/> : <BriefcaseBusiness size={20}/>}</span><div className="card-actions"><button className="icon-button" aria-label={`Edit ${itemTitle(item)}`} onClick={() => setEditing({kind:item.kind,item})}><Pencil size={15}/></button><button className="icon-button" aria-label={`Delete ${itemTitle(item)}`} onClick={() => setDeleting(item)}><Trash2 size={15}/></button></div></div><h3>{itemTitle(item)}</h3><p className="item-subtitle">{itemSubtitle(item)}</p>{item.kind==='skills'&&<div className="skill-learning"><span className="pill">{String(item.data.level||'No level set')}</span>{item.data.target_level&&<p>Working toward <strong>{String(item.data.target_level)}</strong>{item.data.target_date?' · '+item.data.target_date:''}</p>}{item.data.learning_notes&&<p>{String(item.data.learning_notes)}</p>}</div>}{item.kind==="achievements"&&<BulletFeedback text={String(item.data.statement)}/>}{(item.data.start_date || item.data.date || item.data.year) && <span className="item-date">{String(item.data.start_date || item.data.date || item.data.year)}{item.data.current ? ' — Present' : item.data.end_date ? ` — ${item.data.end_date}` : ''}</span>}{item.data.description && <p className="item-description">{String(item.data.description)}</p>}{Array.isArray(item.data.bullets) && <ul className="career-bullets">{(item.data.bullets as {id:string,text:string}[]).map(b => <li key={b.id}>{b.text}</li>)}</ul>}<div className="chips">{((item.data.technologies || item.data.skills || []) as string[]).map(t => <span className="chip" key={t}>{t}</span>)}</div>{typeof item.data.url === 'string' && /^https?:\/\//.test(item.data.url) && <a className="text-link" href={item.data.url} target="_blank" rel="noreferrer">View project <ArrowUpRight size={13}/></a>}</article>)}</div>{!visible.length && <div className="empty-state panel"><div className="empty-icon"><Plus size={25}/></div><h3>A place for your {labels[tab].toLowerCase()}.</h3><p>Add only what matters to your career. Reuse it across your resumes.</p><button className="button secondary" onClick={() => setEditing({kind:tab})}>Add your first entry</button></div>}</>}
    {github&&<GitHubImport onClose={()=>setGithub(false)} onSaved={()=>{refresh();setNotice('Reviewed project imported')}}/>}
