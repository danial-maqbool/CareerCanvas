  ...
  return <div className="app-shell">
    ...
    <main className="main"><header className="topbar"><span>Workspace <span className="muted">/</span> <strong>{page}</strong></span><span className="local-label"><span className="status-dot"/>Private by default</span></header>
      <div className="page-content"><div className="page-heading"><div><span className="eyebrow">A LITTLE CLARITY. A LOT OF POSSIBILITY.</span><h1>{page === 'Dashboard' ? 'Your next chapter starts here.' : page}</h1><p className="muted">One thoughtful space for your career, from first draft to next opportunity.</p></div><span className="outline-label">WORKSPACE FOUNDATION</span></div>
        <section className="welcome-panel"><div><span className="pill"><Sparkles size={14}/> MADE FOR YOUR NEXT MOVE</span><h2>Build your career workspace.</h2><p>Bring your experience together. Shape a resume that feels like you.<br/>Make room for what comes next.</p><p className="foundation-note">The application foundation is ready. Career data and editing features are being implemented.</p></div><div className="paper-illustration" aria-hidden="true"><span className="paper-line title-line"/><span className="paper-line short-line"/><hr/><span className="paper-line"/><span className="paper-line"/><span className="paper-line short-line"/><hr/><span className="paper-line"/><span className="paper-line"/><span className="paper-line"/><div className="paper-seal"><Leaf size={24}/></div></div></section>
