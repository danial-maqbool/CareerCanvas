...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><a className="brand" href="/"><span className="brand-symbol"><Leaf size={22}/></span>CareerCanvas<span className="brand-dot">.</span></a>
      ...
      <nav aria-label="Main navigation">{navigation.map(([name, Icon]) => <button key={name} aria-label={name} aria-current={page === name ? 'page' : undefined} className={`nav-item ${page === name ? 'active' : ''}`} onClick={() => setPage(name)}><Icon size={18}/><span>{name}</span></button>)}</nav>
